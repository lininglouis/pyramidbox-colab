    ...
    for epoch in range(start_epoch, cfg.EPOCHES):
        ...
        for batch_idx, (images, face_targets, head_targets, info) in enumerate(train_loader):
            ...
            if args.cuda:
                ...
                with torch.no_grad():
                    face_targets = [Variable(ann.cuda()) for ann in face_targets]
                    head_targets = [Variable(ann.cuda()) for ann in head_targets]
            else:
...
def val(epoch,
    ...
    step = 0
    t1 = time.time()
    for batch_idx, (images, face_targets, head_targets, info) in enumerate(val_loader):
