...
class WIDERDetection(data.Dataset):
    ...
    def __getitem__(self, index):
        img, face_target, head_target, info = self.pull_item(index)
        return img, face_target, head_target, info
    ...
    def pull_item(self, index):
        ...
        while True:
            ...
            for b in boxes:
                if b[2] > im_width or (b[3]>im_height):
                    print(np.array(self.boxes[index]), im_width, im_height, boxes)
            label = np.array(self.labels[index])

            bbox_labels = np.hstack((label[:, np.newaxis], boxes)).tolist()
            # info.append([im_width, im_height])
            # info.append(self.boxes[index])
        ...
        return torch.from_numpy(img), face_target, head_target, info
...
def detection_collate(batch):
    ...
    return torch.stack(imgs, 0), face_targets,head_targets, sample[3]
...
if __name__ == '__main__':
    ...
    os.chdir('../')
    dataset = WIDERDetection("C:/Users/ESS User/Documents/GitHub/wji_data/Pyramidbox.pytorch-colab/data//face_train.txt")
